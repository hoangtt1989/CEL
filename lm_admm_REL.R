...
CEL_lm_admm_outlier <- function(X, y, beta_test = NULL, F_reparam = NULL, s_hat_reparam = NULL, detect_outlier = T, q = NULL,
                                outlier_loop_arg = list(), wts_init = NULL, delta_plus = F,
  ...
  for (j in 1:outer_maxit) {
    ...
    for (wts_beta_iter in 1:wts_beta_rep) {
      if (wts_beta_order) {
        #######mirror descent
        A_max <- dual_step * (max(abs(crossprod(Xtr_R))) * n + max(abs(crossprod(Xtr_R) %*% delta_curr))) ##t(Xtr_R) %*% Xtr_R
        mirror_step <- 1 / (A_max + max(abs(crossprod(gamma_curr, Xtr_R))))
        if (delta_plus) {
          delta_inp <- -delta_curr
        } else {
          delta_inp <- delta_curr
        }
        mirror_res <- do.call('mirror_descent_outlier_lm_AL_cpp', c(list(wts = wts_new, delta = delta_inp, mirror_step = mirror_step), fun_grad_arg, mirror_arg), quote = T)
        ...
        #######
        #######outlier update
        outlier_res <- do.call('outlier_loop', c(list(wts_new, X, R_curr, gamma_curr, dual_step, delta_new = delta_curr, q = q, delta_plus = delta_plus), outlier_loop_arg), quote = T)
        delta_new <- outlier_res$delta_new
        # R_new <- beta_outlier_res$R_adj_new
        # Xtr_R <- t(R_new * X)
        # beta_converged <- outlier_res$converged
        ...
        mirror_step <- 1 / (A_max + max(abs(crossprod(gamma_curr, Xtr_R))))
        mirror_res <- do.call('mirror_descent_outlier_lm_AL_cpp', c(list(wts = wts_new, delta = delta_new, mirror_step = mirror_step), fun_grad_arg, mirror_arg), quote = T)
        # mirror_res <- do.call('mirror_descent', c(list(fun = fun, grad = grad, wts = wts_new, fun_arg = fun_grad_arg, grad_arg = fun_grad_arg, mirror_step = mirror_step), mirror_arg), quote = T)
        wts_new <- as.vector(mirror_res$wts)
    ...
    #######gamma update
    if (delta_plus) {
      gamma_new <- gamma_curr + dual_step * Xtr_R %*% (wts_new + delta_new)
    } else {
      gamma_new <- gamma_curr + dual_step * Xtr_R %*% (wts_new - delta_new)
    }
    ...
    if (primal_dual_flag) {
      if (wts_beta_order) { ##wts were updated first
        ...
        dual_resid <- sqrt(sum((dual_step * tcrossprod(R_curr * X) %*% (delta_diff))^2))
    ...
    if (primal_dual_flag) {
      if (delta_plus) {
        primal_resid <- sqrt(sum((Xtr_R %*% (wts_new + delta_new))^2))
      } else {
        primal_resid <- sqrt(sum((Xtr_R %*% (wts_new - delta_new))^2))
      }
      # wts_X <- (wts_new - delta_new) * X
      wts_X <- wts_new * X
      delta_X <- delta_new * X
      primal_resid_scale <- max(sqrt(sum((crossprod(wts_X, y))^2)), sqrt(sum((crossprod(wts_X, X %*% beta_new))^2)), 
                                sqrt(sum((crossprod(delta_X, y))^2)), sqrt(sum((crossprod(delta_X, X %*% beta_new))^2)))
      # primal_resid_scale <- max(sqrt(sum((crossprod(wts_X, y))^2)), sqrt(sum((crossprod(wts_X, X %*% beta_new))^2)))
    }
    #######
  ...
  return(list(logelr = logelr, logelr_stat = -2 * logelr, wts = wts_curr / n, sum_wts = sum(wts_curr) / n,
              gamma = gamma_curr, beta_opt = beta_curr, delta_opt = delta_new,

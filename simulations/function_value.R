...
for (sim_it in 1:nsim) {
  ...
  res_ADMM[[sim_it]] <- CEL_lm_beta_fixed(beta_true[1], 1, X, y, algorithm = 'ADMM', outer_maxit = ADMM_maxit, vary_penalty = 'RB', outer_eps = 1e-8,
  ...
  res_nested[[sim_it]] <- CEL_lm_beta_fixed(beta_true[1], 1, X, y, algorithm = 'nested')
...
primal_ADMM <- map_dbl(res_ADMM, 'primal_resid')
...
comb_df2 <- data_frame(fval_diff = (-logelr_ADMM - (-logelr_nested)), primal_diff = primal_ADMM/n - primal_nested) %>% 
  gather(key, value, fval_diff, primal_diff) %>% 
  mutate(key = factor(key, labels = c('Log EL ratio', 'Primal residual')))
ggplot(comb_df2, aes(x = '', y = value)) +
  geom_boxplot(fatten = 3) +
  labs(x = '', y = 'BICEP - Nested') +
  facet_wrap(~ key, scales = 'free_y') +
  theme_bw()


diff_df <- data_frame(fval_diff = (-logelr_ADMM - (-logelr_nested)))
ggplot(diff_df, aes(x = '', y = fval_diff)) +
  geom_boxplot(fatten = 3) +
  labs(x = '', y = 'BICEP - Nested') +
  theme_bw()

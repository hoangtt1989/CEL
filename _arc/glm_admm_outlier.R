...
CEL_glm_admm_outlier <- function(beta_test, X, y, F_reparam = NULL, s_hat_reparam = NULL, prox_fun = NULL, prox_fun_arg = list(),
                                 q = floor(length(y) * .1), 
  ...
  n <- prob_size[1]
  p <- prob_size[2]
  X <- cbind(X, diag(n)) ###make X into [X I]
  beta_curr <- c(beta_test, rep(0, n)) ####make beta into [beta delta]^T
  if (is.null(wts_init)) {
    wts_init <- rep(1, n)
  } else {
  ...
  for (j in 1:outer_maxit) {
    ...
    for (wts_beta_iter in 1:wts_beta_rep) {
      if (wts_beta_order) {
        ...
        beta_res <- do.call( 'proximal_gradient_outlier', c(list(beta_curr, X, y, wts_new, gamma_curr, dual_step, family, prox_fun, q = q), prox_fun_arg, prox_optim_arg))
        beta_new <- beta_res$beta_new
        R_new <- beta_res$R_new
        Xtr_R <- t(R_new * X)

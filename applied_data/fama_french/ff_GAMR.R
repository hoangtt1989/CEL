...
ff_df <- read_csv('applied_data/fama_french/Global_5_Factors_Daily.csv', skip = 6) %>% 
  clean_names() %>% 
  rename(date = x1)
...
fund_df <- read_csv('applied_data/fama_french/GAMR.csv') %>% 
  clean_names()
...
boot_trace_ADMM <- mclapply(1:ncol(design_mat), CEL_lm_beta_boot, design_mat, y, B = B, parallel = F,
                                                                              algorithm = 'ADMM', dual_step = 20, RB_tau = 1.5, wts_beta_rep = 3,

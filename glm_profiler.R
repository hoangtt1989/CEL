...
CEL_glm_boot <- function(X, y, prox_fun, prox_fun_arg, family = c('gaussian', 'binomial', 'poisson'), ..., beta_init = NULL, algorithm = c('ADMM', 'nested'), B = 200, prob = .95, parallel = T, seed = 123) {
  ##checking inputs
  family <- family[1]
  algorithm <- algorithm[1]
  if (!(algorithm %in% c('ADMM'))) {
    stop('Only ADMM is currently implemented')
  ...
  if (is.null(beta_init)) {
    beta_init <- switch(family,
                        ...
                        binomial = glm.fit(X, y, family = binomial(link = 'logit'))$coefficients,
                        poisson = glm.fit(X, y, family = poisson(link = 'log'))$coefficients)
  }
  set.seed(seed)
  if (parallel) {
    `%fun%` <- doRNG::`%dorng%`
  ...
  init_res <- CEL_glm_admm(beta_init, X, y, prox_fun = prox_fun, prox_fun_arg = prox_fun_arg, family = family, ...)
  start_time <- Sys.time()
  boot_res <- foreach::foreach(i = 1:B, .combine = cbind) %fun% {
    ...
    beta_init <- switch(family,
                        ...
                        binomial = glm.fit(X, y, family = binomial(link = 'logit'))$coefficients,
                        poisson = glm.fit(X, y, family = poisson(link = 'log'))$coefficients)
    CEL_res <- try(CEL_glm_admm(beta_init, X, y, prox_fun = prox_fun, prox_fun_arg = prox_fun_arg, family = family, ...))
    # if (class(CEL_res) == "try-error") {
    #   stat <- Inf
    # } else {
    #   stat <- CEL_res$logelr_stat
    # }
    stat <- ifelse(class(CEL_res) == 'try-error', Inf, CEL_res$logelr_stat)
    return(stat)
  }
  end_time <- Sys.time()
...
CEL_glm_beta_boot <- function(fix_index, X, y, family = c('gaussian', 'binomial', 'poisson'), B = 200, prob = .95, mean_init = NULL, algorithm = c('ADMM', 'nested'), parallel = T, seed = 123, ...) {
  ##checking inputs
  family <- family[1]
  algorithm <- algorithm[1]
  if (!(algorithm %in% c('ADMM'))) {
...
glm_boot_beta_interm_fun <- function(fix_index, alpha_test, X, y, F_reparam, s_hat_reparam, algorithm = c('ADMM', 'nested'), family, ...) {
  algorithm <- algorithm[1]
  ##resample data
  n <- nrow(X)
  sample_ind <- sample(1:n, n, replace = T)
  ...
  res <- switch(algorithm,
                ADMM = CEL_glm_admm(beta_test, X, y, F_reparam, s_hat_reparam, family = family, ...),
                nested = CEL_glm_nested(beta_test, X, y, F_reparam, s_hat_reparam, family = family, ...))
  ##
...
CEL_glm_beta_fixed <- function(beta_fix_val, fix_index, X, y, family = c('gaussian', 'binomial', 'poisson'), mean_init = NULL, algorithm = c('ADMM', 'nested'), ...) {
  ##checking inputs
  family <- family[1]
  algorithm <- algorithm[1]
  if (!(algorithm %in% c('ADMM'))) {
  ...
  CEL_glm_beta_add(alpha_add, fix_index, X, y, family, mean_init, algorithm, ...)
  ##
}
##################

##################compute EL for a fixed value of a component of the beta vector (alpha_add specifies how far away from MLE) for glm
CEL_glm_beta_add <- function(alpha_add, fix_index, X, y, family = c('gaussian', 'binomial', 'poisson'), mean_init = NULL, algorithm = c('ADMM', 'nested'), ...) {
  ##checking inputs
  family <- family[1]
  algorithm <- algorithm[1]
  if (!(algorithm %in% c('ADMM'))) {
...
##################find the EL confidence interval for a component of the beta vector for glm using ADMM
CEL_glm_beta_profiler <- function(fix_index, X, y, family = c('gaussian', 'binomial', 'poisson'), conf_level = .05, test_thresh = 'chisq', upper_break = NULL, lower_break = 1e-6, upper_divide = 2, upper_increase = 1e-1, algorithm = c('ADMM', 'nested'), ...) {
  ##checking inputs
  family <- family[1]
  algorithm <- algorithm[1]
  ...
  root_fun <- function(inp) {
    ret <- CEL_glm_beta_add(inp, fix_index, X, y, mean_init = beta_MLE, algorithm = algorithm, family = family, ...)
...
# CEL_glm_pred_fixed <- function(alpha_add, fix_index, X, y, mean_init = NULL, algorithm = c('ADMM', 'nested'), ...) {
#   ##checking inputs
#   algorithm <- algorithm[1]
#   if (!(algorithm %in% c('ADMM', 'nested'))) {
#     stop('supply a valid algorithm - ADMM or nested')
...
#   ##run the chosen algorithm
#   res <- switch(algorithm,
#                 ADMM = CEL_glm_admm(beta_test, X, y, F_reparam, s_hat_reparam, ...),
#                 nested = CEL_glm_nested(beta_test, X, y, F_reparam, s_hat_reparam, ...))
#   ##
...
# CEL_glm_profiler <- function(fix_index, X, y, type = c('beta', 'prediction'), conf_level = .05, mean_val = NULL, upper_break = NULL, lower_break = 1e-6, upper_divide = 2, upper_increase = 1e-1, algorithm = c('ADMM', 'nested'), ...) {
#   ##checking inputs
#   type <- type[1]
...
#   root_fun <- function(inp) {
#     ret <- switch(type,
#                   beta = CEL_glm_beta_add(inp, fix_index, X, y, mean_init = beta_OLS, algorithm = algorithm, ...),
#                   prediction = CEL_glm_pred_fixed())
#     ret$logelr_stat - test_thresh
#   }
#   ##
#   #####make sure there is a sign change in the interval
...
# CEL_glm_CI <- function(X, y, type = c('beta', 'prediction'), conf_level = .05, algorithm = c('ADMM', 'nested'), upper_break = NULL, lower_break = 1e-6, upper_divide = 2, upper_increase = 1e-1, ...) {
#   ##checking inputs
#   type <- type[1]
#   if (!(type %in% c('beta', 'prediction'))) {
#     stop('supply a valid interval type - beta or prediction')

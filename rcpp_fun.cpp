...
// [[Rcpp::export]]
double fun_outlier_lm_AL_cpp(const VectorXd & wts, //outlier version
  ...
  return(-wts.array().log().sum() + (Xtr_R * (wts - delta)).dot(gamma) + .5 * dual_step * (Xtr_R * (wts - delta)).array().pow(2).sum());
}

// [[Rcpp::export]]
VectorXd grad_outlier_lm_AL_cpp(const VectorXd & wts, //outlier version
                        const VectorXd & delta,
                        const VectorXd & gamma, 
                        const MatrixXd & X, 
                        const VectorXd & R, 
                        const double & dual_step) {
  MatrixXd Xtr_R = (R.asDiagonal() * X).transpose();
  return(-wts.array().inverse().matrix() + Xtr_R.adjoint() * gamma + dual_step * Xtr_R.adjoint() * (Xtr_R * (wts - delta)));
}


// [[Rcpp::export]]
double fun_lm_AL_cpp(const VectorXd & wts, 
                     const VectorXd & gamma, 
                     const MatrixXd & X, 
                     const VectorXd & R, 
                     const double & dual_step) {
...
// [[Rcpp::export]]
VectorXd grad_lm_AL_cpp(const VectorXd & wts, 
                        const VectorXd & gamma, 
                        const MatrixXd & X, 
                        const VectorXd & R, 
                        const double & dual_step) {
...
// [[Rcpp::export]]
VectorXd closed_beta_lm_AL_cpp(const Map<VectorXd> & wts, 
                               const Map<VectorXd> & gamma, 
                               const Map<VectorXd> & y, 
                               const Map<MatrixXd> & X, 
                               const Map<MatrixXd> & F_reparam, 
                               const Map<VectorXd> & s_hat_reparam,
                               const double & dual_step) {
...
// [[Rcpp::export]]
List mirror_descent_lm_AL_cpp(const Map<VectorXd> & wts, 
                              const Map<VectorXd> & gamma, 
                              const Map<MatrixXd> & X, 
                              const Map<VectorXd> & R, 
                              const double & dual_step,
...
// [[Rcpp::export]]
List mirror_descent_outlier_lm_AL_cpp(const Map<VectorXd> & wts,
                              const Map<VectorXd> & delta,
                              const Map<VectorXd> & gamma, 
                              const Map<MatrixXd> & X, 
                              const Map<VectorXd> & R, 
                              const double & dual_step,
  ...
  double tiny2 = std::pow(tiny, .25);
  if (!tol_type.compare("fval")) {
    fval_curr = fun_outlier_lm_AL_cpp(wts_curr, delta, gamma, X, R, dual_step);
  } else if (!tol_type.compare("logelr")) {
    logelr_curr = wts_curr.array().log().sum();
  }
  double tol;
  ...
  for (i = 1; i <= maxit; ++i) {
    theta_curr = 2.0 / (i + 1.0);
    theta_new = 2.0 / (i + 2.0);
    grad_update = grad_outlier_lm_AL_cpp(y_curr, delta, gamma, X, R, dual_step);
    if (vary_step && !line_search) {
      mirror_step *= std::pow(i, -.5);
    }
    nu_new = (nu_curr.array() * exp((-mirror_step * grad_update / theta_curr).array())).matrix();
    ...
    //line search
    if (line_search) {
      fval_new = fun_outlier_lm_AL_cpp(wts_new, delta, gamma, X, R, dual_step);
      fval_y = fun_outlier_lm_AL_cpp(y_curr, delta, gamma, X, R, dual_step);
      surr_curr = fval_y + grad_update.dot(wts_new - y_curr) + std::pow(mirror_step, -1.0) * bregman_entropy_cpp(wts_new, y_curr);
      ls_converged = fval_new < surr_curr;
      if (!ls_converged || isinf(ls_converged) || isnan(ls_converged)) {
        for (iter_ls = 0; iter_ls < maxit; ++iter_ls) {
